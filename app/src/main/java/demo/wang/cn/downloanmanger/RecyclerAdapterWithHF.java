...
public class RecyclerAdapterWithHF extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void addHeaders(List<View> headers) {
        mHeaders.clear();
    ...
    public boolean containerFootView(View header) {
        if (mFooters == null && mFooters.size() <= 0) {
            return false;
        }
        return mFooters.contains(header);
    }
     ...
     * add a header to the adapter
     */
    public void addFooter(int index, View header) {
        try {
            if (!mFooters.contains(header) && index <= mFooters.size()) {
                mFooters.add(index, header);
                // animate
    ...
    public void addFooters(List<View> foots) {
        mFooters.clear();
        mFooters.addAll(foots);
        notifyDataSetChangedHF();
    }
    ...
    public void setLongClickDisable(boolean disable) {
        this.longClickDisable = disable;
    }
}
